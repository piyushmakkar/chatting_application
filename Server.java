...
public class Server implements ActionListener{
	...
	static JFrame f1 = new JFrame();//static variable to call validate function in static main
...
Server(){
		...
		ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("chat/icons/1.png"));
		...
		a1 = new JPanel();
		//a1.setBounds(0,50,285,270); set in scroll bar
		a1.setFont(new Font("SAN_SERIF",Font.PLAIN,14));
		//f1.add(a1); added in scroll bar
		...
		t1.addKeyListener(new KeyAdapter()  //works on movements of keys
		{
		...
		f1.setLocation(300,100); //defines position of frame on the screen.
	...
	public static JPanel formatLabel(String out)//to format label on new messages {
	{
		...
		JLabel l1 = new JLabel("<html><p style = \"width : 70px\">"+out+"</p></html>"); // to add label on message //html tags used so that long messages get break into parts
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			while(true)
			{
				...
				vertical.add(left);
				f1.validate(); // to refresh or load the visible true part on every message
			}
